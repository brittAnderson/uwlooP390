...
## Packages
Unlike R, you do not install python packages from within an interpreted environment. Worse, the python package system has differnt tools. Recently, `pip` has been the most commonly used, and there is an interface to it through `uv`. Conflicting packages are a real risk. Use a virtual environment. Also, available via the `uv` interface. 
